...
define('LOG_PATH_SPE', \think\facade\Env::get('root_path') . 'public/log/');
...
class Debug
{
     ...
     */
    public static function write($msg, $preFix = 'log', $level = 'DEBUG', $type = null, $path = "")
    {
        $msg     = date('[ Y-m-d H:i:s ]') . "[{$level}]\r\n" . $msg . "\r\n";
        $logPath = LOG_PATH_SPE . $path . "/";
        if (!is_dir($logPath)) {
            mkdir($logPath);
        }
        if ($type) $logPath .= '.' . $type;
        $logPath .= $preFix . '_' . date('Y_md_H_i');
        $logPath .= '.log';
        touch($logPath);
    ...
    {
        $msg     = date('[ Y-m-d H:i:s ]') . "[{$level}]\r\n" . $msg . "\r\n";
        $logPath = $logPath . 'clilogs' . self::$moduleName . '/' . $preFix . '_' . date('YmdHi');
        //echo $logPath;
        if ($type)
            $logPath .= '.' . $type;
     ...
     * @param null   $type
     */
    public static function r($msg, $preFix = 'log', $logPath = LOG_PATH_SPE, $level = 'SYSTEM', $type = null)
    {
     ...
     * 打印debug日志
     *
     * @param        $arr array $msg 日志信息
     * @param string $preFix 日志前缀，配合appname使用，可以清晰的知道日志来源
     * @param null   $type
     * @param string $path 写入日志的地址
     */
    public static function debugArr($arr = null, $preFix = 'log', $type = null, $path = "")
    {
        ...
        self::write($msg, $preFix, 'DEBUG', $logType, $path);
    }
