<?php /*a:2:{s:73:"/www/wwwroot/stock.ecou.vip/application/dash/view/agent/withdraw_log.html";i:1543393320;s:66:"/www/wwwroot/stock.ecou.vip/application/dash/view/base/layout.html";i:1665562605;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
                ...
                <div class="box box-info">
                    ...
                    <!--div class="box-body no-padding">
                        <table class="table table-bordered table-middle">
                            <tr>
                                <td>
                                    总申请提现金额：<?php echo !empty($totalWithdraw['money']) ? htmlentities($totalWithdraw['money']) : 0; ?>&nbsp;&nbsp;&nbsp;&nbsp;
                                    总成功提现金额：<?php echo !empty($successWithdraw['money']) ? htmlentities($successWithdraw['money']) : 0; ?>&nbsp;&nbsp;&nbsp;&nbsp;
                                    总提现中金额：<?php echo !empty($adminCheckedWithdraw['money']) ? htmlentities($adminCheckedWithdraw['money']) : 0; ?>
                                </td>
                            </tr>
                        </table>
                    </div-->
...
function manualPay(id) {
    layer.confirm('确定要手动支付吗？', {
        ...
        if (id == "") {
            ...
            $.post("<?php echo url('stock/orgWithdraw/manualPay'); ?>", {id: id}, function (data) {
                if (data.code == 1) {
                    layer.msg(data.msg, {icon: 1, time: 1500, shade: .3, shadeClose: true}, function () {
                        window.location.href = "<?php echo url('withdraw_log'); ?>";
                    });
                } else {
...
function doPay(id) {
    layer.confirm('确定要代付吗？', {
        ...
        if (id == "") {
            ...
            $.post("<?php echo url('stock/orgWithdraw/doPay'); ?>", {id: id}, function (data) {
                if (data.code == 1) {
                    layer.msg(data.msg, {icon: 1, time: 1500, shade: .3, shadeClose: true}, function () {
                        window.location.href = "<?php echo url('withdraw_log'); ?>";
                    });
