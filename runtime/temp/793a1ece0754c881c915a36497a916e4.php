<?php /*a:2:{s:67:"/www/wwwroot/stock.ecou.vip/application/dash/view/notice/index.html";i:1560501048;s:66:"/www/wwwroot/stock.ecou.vip/application/dash/view/base/layout.html";i:1665562605;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
            ...
            <div class="col-sm-12">
                <div class="box box-info">
                    <div class="box-header">
                        <a href="<?php echo url('notice/notice_edit'); ?>" class="btn btn-success"><i class="fa fa-plus"></i> 添加公告</a>
                    </div>
                    <div class="box-body">
                        <table class="table table-bordered table-middle table-center">
                            ...
                            <tbody>
                            <?php foreach($noticeList as $k=>$nl): ?>
                                <tr>
                                    <td><?php echo htmlentities($k+1); ?></td>
                                    <td><a href="<?php echo url('notice/notice_edit', ['id'=>$nl['id']]); ?>" class="name"><?php echo htmlentities($nl['title']); ?></a></td>
                                    <td><?php echo !empty($nl['state']) ? '已发布' : '未发布'; ?></td>
                                    <td>
                                        <a class="btn btn-sm btn-primary" href="#" onclick="changeState('<?php echo htmlentities($nl['id']); ?>');"><?php echo !empty($nl['state']) ? '取消发布' : '确认发布'; ?></a>
                                        <a class="btn btn-sm btn-primary" href="<?php echo url('notice/notice_edit', ['id' => $nl['id']]); ?>">编辑</a>
                    ...
                    <div class="box-footer">
                        <div class="col-sm-12 no-padding"><?php echo $noticeList->render(); ?></div>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    function delNews(v) {
        layer.confirm('确定要删除吗？', {
            ...
            if(id == "") {
                ...
                $.post("<?php echo url('stock/notice/noticeDel'); ?>", {id:id}, function(data){
                    if(data.code == 1){
    ...
    function changeState(id) {
        if (id == '') {
            ...
            $.post("<?php echo url('stock/notice/changeState'); ?>", {id: id}, function (data) {
                if (data.code == 1) {
                    layer.msg(data.msg, {icon: 1, time: 1500, shade: .3, shadeClose: true}, function () {
                        window.location.href = "<?php echo url('index'); ?>";
                    });
