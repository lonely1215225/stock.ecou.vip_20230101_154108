<?php /*a:2:{s:72:"/www/wwwroot/stock.ecou.vip/application/dash/view/user/recharge_log.html";i:1665558231;s:66:"/www/wwwroot/stock.ecou.vip/application/dash/view/base/layout.html";i:1665562605;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
            ...
            <div class="col-sm-12">
                <div class="box box-info">
                    <div class="box-header with-border">
                        <form id="searchInfo" action="<?php echo url('recharge_log'); ?>" method="get" autocomplete="off">
                                ...
                                <div class="search-box">
                                    ...
                                    <div class="search-body">
                                        <select class="form-control" name="pay_state" id="payState">
                                            ...
                                            <?php foreach($rechargePayStateList as $k=>$v): ?>
                                            <option value="<?php echo htmlentities($k); ?>" <?php echo isset($pay_state) && $k==$pay_state?'selected':''; ?>><?php echo htmlentities($v); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="search-box">
                                    ...
                                    <div class="search-body">
                                        <select class="form-control" name="pay_company">
                                            <option value="">请选择</option>
                                            <?php foreach($payCompanyList as $v): ?>
                                            <option value="<?php echo htmlentities($v['id']); ?>" <?php echo isset($pay_company) && $v['id']==$pay_company?'selected':''; ?>><?php echo htmlentities($v['name']); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                </div>
                    ...
                    <div class="box-body no-padding table-scroll-x">
                        <table class="table table-bordered table-middle table-center">
                            ...
                            <tr class="tj">
                                <td>总计</td>
                                <td colspan="2"></td>
                                <td><?php echo !empty($totalMoney['money']) ? htmlentities($totalMoney['money']) : '0'; ?></td>
                                <td colspan="8"></td>
                            </tr>
                            </thead>
                            <tr>
                            ...
                            <?php foreach($useRecharge as $k=>$v): ?>
                            <tr>
                                <td>
                                ...
                                <td><?php echo htmlentities($v['offline_name']); ?></td>
                                <td><?php echo htmlentities($v['money']); ?></td>
                                <td><?php echo htmlentities($v['real_money']); ?></td>
                                <td><?php echo !empty($v['pay_state']) ? htmlentities($rechargePayStateList[$v['pay_state']]) : ''; ?></td>
                                <td><?php echo !empty($v['pay_time']) ? date('Y-m-d H : i:s',$v['pay_time']):''; ?></td>
                                <td><?php echo htmlentities($v['third_order_sn']); ?></td>
                                <td>
                                    <?php if(isset($payCompanyList[$v['pay_company_id']]['name'])): ?>
                                    <?php echo htmlentities($payCompanyList[$v['pay_company_id']]['name']); ?><?php echo !empty($payCompanyList[$v['pay_company_id']]['pay_type']) ? '('.$payCompanyList[$v['pay_company_id']]['pay_type'].')' : ''; ?>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php echo !empty($v['agent_name']) ? htmlentities($v['agent_name']) : ''; ?>
                                    <br>
                                    <?php echo !empty($v['broker_name']) ? htmlentities($v['broker_name']) : ''; ?>
                                </td>
                                <td><?php echo htmlentities($v['create_time']); ?></td>
                                <td>
                                    <?php if(isset($payCompanyList[$v['pay_company_id']]['pay_channel'])): if($payCompanyList[$v['pay_company_id']]['pay_channel'] == 'online' && ($v['pay_state'] == 'wait' || $v['pay_state'] == 'failed')): ?>
                                        <a class="btn btn-sm btn-primary" href="#" onclick="openDialog(<?php echo htmlentities($v['id']); ?>,<?php echo htmlentities($v['user_id']); ?>);">手动入账</a>
                                        <?php endif; if($payCompanyList[$v['pay_company_id']]['pay_channel'] != 'online' && ($v['pay_state'] == 'wait' || $v['pay_state'] == 'failed')): ?>
                                        <a class="btn btn-sm btn-primary" href="#" onclick="openBankcard(<?php echo htmlentities($v['id']); ?>,<?php echo htmlentities($v['user_id']); ?>);">线下入账</a>
                                        <?php endif; if($payCompanyList[$v['pay_company_id']]['pay_channel'] != 'online' && $v['pay_state'] == 'offline'): ?>
                                        <a class="btn btn-sm btn-primary" href="#" onclick="openDetail(<?php echo htmlentities($v['id']); ?>);">详情</a>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <a class="btn btn-sm btn-primary mb5" href="#" onclick="delItem('<?php echo htmlentities($v['id']); ?>')">删除</a>
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </table>
                    ...
                    <div class="box-footer">
                        <div class="col-sm-12 no-padding"><?php echo $useRecharge->render(); ?></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function manual(id,userId,money,third_order_sn) {
        $.post("<?php echo url('stock/userRecharge/manualByAdmin'); ?>", {
            id:id,
            money: money,
            ...
            if (data.code == 1) {
                layer.msg(data.msg, {icon: 1, time: 1500, shade: .3, shadeClose: true}, function () {
                    window.location.href = "<?php echo url('recharge_log'); ?>";
                });
            } else {
    ...
    function offlineTransfer(id,userId,money) {
        $.post("<?php echo url('stock/userRecharge/offlineTransfer'); ?>", {
            id:id,
            user_id:userId,
            money:money,
        }, function (data) {
    ...
    function delItem(v) {
        layer.confirm('确定要删除吗？', {
            ...
            if(v == "") {
                ...
                $.post("<?php echo url('stock/userRecharge/delete'); ?>", {id:v}, function(data){
                    if(data.code == 1){
                        layer.msg(data.msg, {icon:1, time:1500, shade:.3, shadeClose:true}, function(){
                            window.location.href = "<?php echo url('recharge_log'); ?>";
                        });
