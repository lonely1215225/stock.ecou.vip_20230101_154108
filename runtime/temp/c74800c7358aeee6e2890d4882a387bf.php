<?php /*a:2:{s:72:"/www/wwwroot/stock.ecou.vip/application/dash/view/news/contact_edit.html";i:1540864306;s:66:"/www/wwwroot/stock.ecou.vip/application/dash/view/base/layout.html";i:1665562605;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<link rel="stylesheet" href="/static/dash/lib/summernote/summernote.css">
<script src="/static/dash/lib/summernote/summernote.js"></script>
                    ...
                    <form id="addForm" action="" method="post" class="form-horizontal">
                        <div class="box-body">
                            <!--div class="form-group col-sm-8">
                                ...
                                <div class="col-sm-8">
                                    <input class="form-control" name="title" id="title" value="<?php echo !empty($contactInfo['title']) ? htmlentities($contactInfo['title']) : ''; ?>">
                                </div>
                            </div-->
                            ...
                            <div class="form-group col-sm-8">
                                ...
                                <div class="col-sm-8">
                                    <textarea class="form-control"  id="content" name="content"><?php echo !empty($contactInfo['content']) ? htmlentities($contactInfo['content']) : ''; ?></textarea>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    $(function () {
        ...
        function sendFile($summernote, file) {
            ...
            $.post({
                url: "<?php echo url('stock/article/uploadImg'); ?>",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
        ...
        $("#addForm").submit(function(){
            ...
            var title   = $('#title').val();
            var id      ="<?php echo !empty($contactInfo['id']) ? htmlentities($contactInfo['id']) : 0; ?>";
            if(content == ""){
                layer.msg("请填写联系方式", {icon:2, time:1500, shade:.3, shadeClose:true});
            }else {
                $.post("<?php echo url('stock/article/saveNews'); ?>", {id:id,title:title,cat_id:0,content:content}, function(data){
                    if(data.code == 1){
                        layer.msg(data.msg, {icon:1, time:1500, shade:.3, shadeClose:true}, function(){
                            window.location.href = "<?php echo url('contact'); ?>";
                        });
                    } else {
                        layer.msg(data.msg, {icon:2, time:1500, shade:.3, shadeClose:true});
                    }
