<?php /*a:2:{s:70:"/www/wwwroot/stock.ecou.vip/application/dash/view/risk/black_list.html";i:1654349724;s:66:"/www/wwwroot/stock.ecou.vip/application/dash/view/base/layout.html";i:1665562605;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
            ...
            <div class="col-sm-12">
                <div class="box box-info">
                    ...
                    <div class="box-header with-border">
                        <form id="searchInfo" action="<?php echo url('black_list'); ?>" method="get">
                    ...
                    <div class="box-body no-padding">
                        <table class="table table-bordered table-middle table-center">
                            ...
                            <tr>
                                    ...
                                    <!--td><?php echo !empty($nl['is_margin']) ? '是'  :  '否'; ?></td-->
                                    <td><?php echo !empty($nl['is_black']) ? '是'  :  '否'; ?></td>
                                    <!--td><?php echo htmlentities($riskList[$nl['risk_level']]); ?></td-->
                                    <td><?php echo htmlentities($nl['board_lot']); ?></td>
                                    <td>
                                        <a class="btn btn-sm btn-primary" href="#" onclick="toggleIsBlack('<?php echo htmlentities($nl['id']); ?>','<?php echo htmlentities($nl['stock_code']); ?>','<?php echo htmlentities($nl['market']); ?>');"><?php echo !empty($nl['is_black']) ? '取消禁买' : '加入禁买'; ?></a>
                                    </td>
                            </tr>
                            <?php endforeach; ?>
                            </tbody>
...
function toggleIsBlack(id,stockCode,market, fname) {
    if (id == '') {
        ...
        $.post("<?php echo url('stock/stock/editBlack'); ?>", {id: id,stock_code:stockCode,market:market}, function (data) {
            if (data.code == 1) {
                layer.msg(data.msg, {icon: 1, time: 1500, shade: .3, shadeClose: true}, function () {
                    window.location.href = "<?php echo url('black_list'); ?>";
                });
            } else {
...
function addBlack(checkmarket,checkcode,checkid) {
    if (checkcode == '') {
        ...
        $.post("<?php echo url('stock/stock/addBlack'); ?>", {id:checkid,stock_code: checkcode,market:checkmarket}, function (data) {
            if (data.code == 1) {
                layer.msg(data.msg, {icon: 1, time: 1500, shade: .3, shadeClose: true}, function () {
                    window.location.href = "<?php echo url('black_list'); ?>";
                });
