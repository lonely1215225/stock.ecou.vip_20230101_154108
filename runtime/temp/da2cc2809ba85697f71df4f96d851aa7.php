<?php /*a:2:{s:80:"/www/wwwroot/stock.ecou.vip/application/dash/view/settings/non_trading_date.html";i:1546515158;s:66:"/www/wwwroot/stock.ecou.vip/application/dash/view/base/layout.html";i:1665562605;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
        ...
        <div class="row">
            <div class="col-sm-6">
                <div class="box box-info">
                    <div class="box-header with-border"><b><?php echo htmlentities($month['thisTitle']); ?></b></div>
                        ...
                        <table class="table table-bordered table-middle table-center">
                            ...
                            <tr>
                                ...
                                $thisCount = count($month['this']);
                                 if(is_array($month['this']) || $month['this'] instanceof \think\Collection || $month['this'] instanceof \think\Paginator): $i = 0; $__LIST__ = $month['this'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?>
                                <td data-id="<?php echo htmlentities($d['trading_date']); ?>" class="<?php echo $d['trading_date']=='' ? 'disabled'  :  ($d['is_disabled'] ? 'selected' : 'editable'); ?>">
                                    <?php echo substr($d['trading_date'], 8); ?>
                                </td>
                                <?php if($i % 7 == 0 && $i < $thisCount): ?>
                            </tr>
                            <tr>
                                <?php endif; ?>
            ...
            <div class="col-sm-6">
                <div class="box box-info">
                    <div class="box-header with-border"><b><?php echo htmlentities($month['nextTitle']); ?></b></div>
                    <div class="box-body no-padding">
                        <table class="table table-bordered table-middle table-center">
                            <thead>
                            <tr>
                            ...
                            <tr>
                                ...
                                $nextCount = count($month['next']);
                                 if(is_array($month['next']) || $month['next'] instanceof \think\Collection || $month['next'] instanceof \think\Paginator): $i = 0; $__LIST__ = $month['next'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?>
                                <td data-id="<?php echo htmlentities($d['trading_date']); ?>" class="<?php echo $d['trading_date']=='' ? 'disabled'  :  ($d['is_disabled'] ? 'selected' : 'editable'); ?>">
                                    <?php echo substr($d['trading_date'], 8); ?>
                                </td>
                                <?php if($i % 7 == 0 && $i < $nextCount): ?>
                            </tr>
                            <tr>
                                <?php endif; ?>
                                <?php endforeach; endif; else: echo "" ;endif; ?>
...
$(".selected,.editable").on("click", function () {
    ...
    $(this).addClass('flag');
    $.post("<?php echo url('stock/nonTradingDate/setNonTradingDate'); ?>", {non_trading_date: date}, function (data) {
